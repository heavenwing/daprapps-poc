    ...
    public class DaprWorkflowExecutor : AppCallback.AppCallbackBase
    {
        ...
        private async Task<Any> ExecuteWorkflow(string name, ByteString data)
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.TraceError(ex.InnerException.Message, ex.InnerException);
            }
            return any;
        }
        ...
        public override async Task<TopicEventResponse> OnTopicEvent(TopicEventRequest request, Grpc.Core.ServerCallContext context)
        {
            ...
            Trace.TraceInformation(request.Data.ToStringUtf8());
            var _ = ExecuteWorkflow(request.Topic, request.Data);
